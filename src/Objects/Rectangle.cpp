...
: Object(mass, pos_x, pos_y), dimensions(dimX, dimY)
{
    ...
    sf::Vector2f rectangleCenter(dimX/2, dimY/2);
    shape->setOrigin(rectangleCenter);
...
bool Rectangle::inBounds( sf::Vector2f vector ){
    return ((vector.x >= position_x || vector.x <= position_x+dimensions.first) 
        &&  (vector.y <= position_y || vector.y >= position_y+dimensions.second)) ;
